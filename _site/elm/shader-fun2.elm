...
mesh =
  gridMeshTris 3
               3
               (\attr row col -> {attr | position = vec2 (toFloat row) (toFloat col)})
               { position = vec2 0 0, color = vec3 1 0 1 }
{-
    GL.triangles <|
...
gridMeshTris : Int -> Int -> (attribute -> Int -> Int -> attribute) -> attribute -> GL.Mesh attribute
gridMeshTris rows cols f attr = 
  GL.indexedTriangles (List.map2 (f attr) (upTo rows) (upTo cols)) (tris rows cols)

upTo n = List.range 0 (n-1)
tris rows cols = 
  List.concat <| List.map2 makeQuad (upTo rows) (upTo cols)
makeQuad row col = 
  [(row, row+1, row+col), (row+1, row+1+col, row+col)]
...
view : Model -> Html Msg
view model =
  let gl = GL.toHtml [ width winWidth, height winHeight ]
