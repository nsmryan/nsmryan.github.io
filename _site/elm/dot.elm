...
view model =
    GL.toHtml
        ...
        [ GL.entity vertexShader dotFragmentShader mesh { mat = ortho2D 1 1 , dims = vec2 winWidth winHeight, time = model.currentTime, mousePos = mouseToVec2 model.mousePos, radius = 60.0 } ]
...
dotFragmentShader : GL.Shader {} { u | mousePos : Vec2, dims : Vec2, time : Float, radius : Float } { vcolor : Vec3 }
dotFragmentShader =
    [glsl|
...
void main () {
  ...
  float thresh = 0.5;
  float numSides = 4.0;
  float sideRads = (2.0*3.1415926535879) / numSides;

  vec2 towardsMouse = coord - mousePos;
  float rads = atan2(towardsMouse.y, towardsMouse.x);

  float radsInto = mod(rads, sideRads);

  vec4 red = vec4(1.0, 0.0, 0.0, 1.0);
  vec4 green = vec4(0.0, 1.0, 0.0, 1.0);

  if (length(towardsMouse) * vec2(sin(radsInto), cos(radsInto)) < vec2(10.0, 10.0))
  {
    gl_FragColor = mix(red, green, noise(vec3(0.0, 0.0, time/1000.0)));
  }
  else
  {
